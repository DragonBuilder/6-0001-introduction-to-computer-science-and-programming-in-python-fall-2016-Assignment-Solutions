...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        # make a plain text object with message_text of this object, and no shift
        one_possible_shifted_object = PlaintextMessage(self.message_text, 0, self.wordlist)
        #print('Original message: '+ one_possible_shifted_object.get_message_text_encrypted())

        # how many valid words were found in the best shift done till now, initialized with current values
        MAX_VALID_WORDS_IN_DECRYPT = len(one_possible_shifted_object.get_valid_words())
        #print('Valid words in original message: '+ str(MAX_VALID_WORDS_IN_DECRYPT))

        # tuple, has the best shift value and the decrypted message for that shift value, initialized with current values
        best_decrypt = (one_possible_shifted_object.get_shift(), one_possible_shifted_object.get_message_text())


        # do a shift for each possiblity
        for current_shift in range(1, MAX_SHIFTS):
            #print('shifting by: '+ str(current_shift))
            # shift the message
            one_possible_shifted_object.change_shift(current_shift)

            # total number of walid words found in that
            shifted_text = one_possible_shifted_object.get_message_text_encrypted()

            #print('Shifted message: '+ shifted_text)

            # get the number of valid words in the shifted_text
            total_valid_words_in_current_shift = len(one_possible_shifted_object.compute_valid_words(shifted_text))

            #print('valid words in shifted message: '+ str(total_valid_words_in_current_shift))
            ...
            if total_valid_words_in_current_shift > MAX_VALID_WORDS_IN_DECRYPT :
                # if so, update
                best_decrypt = (one_possible_shifted_object.get_shift(), shifted_text)
